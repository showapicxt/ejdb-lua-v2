...
int ejdb_lua_ejdb_gc(lua_State* L) {
  ...
  if (db && *db) {
    printf("ejdb_lua_ejdb_gc1111111111111111111111111 \n"  );
//    EJDB_LUA_ERROR(ejdb_close(db));
    iwrc rc=ejdb_close(db);
    printf("ffffffffffffffffffffffffffffffffffff %d %s \n",rc,iwlog_ecode_explained(rc)  );
  }
...
//  printf("cccccccccccccccccc %s \n" ,file);
  EJDB_OPTS opts = {.kv = {.path = file, .oflags = mode},.no_wal=false};
//    printf("ddddddddddddddddddddddddd  %d \n",   lua_gettop(L)  );
//    printf("ddddddddddddddddddddddddd   %d \n" ,sizeof(EJDB));
//    EJDB* dbs= lua_newuserdata(L,20000000);
//    lua_pop(L, 1);
    double MemCount = (double)lua_gc(L, LUA_GCCOUNTB, 0) + (double)(lua_gc(L, LUA_GCCOUNT, 0) * 1024);
//    printf("ddddddddddddddddddddddddd MemCount   %d \n" ,MemCount);
...
int ejdb_lua_ejdb_close(lua_State* L) {
  ...
  if(db){
    printf("ejdb_lua_ejdb_close1111 \n");
    EJDB_LUA_ERROR(ejdb_close(db));
  }
...
static int getmillisecond(lua_State *L) {
  struct timeval tv;
  gettimeofday(&tv,NULL);
  long long millisecond= (long long)tv.tv_sec * 1000 + tv.tv_usec/1000;
...
int luaopen_ejdb(lua_State* L) {
  ...
  lua_pushcfunction(L, tttest);
  lua_setfield(L, -2, "tttest");
  lua_pushcfunction(L, test_one);
  lua_setfield(L, -2, "test_one");
  lua_pushcfunction(L, getmillisecond);
